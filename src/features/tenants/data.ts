...
export async function inviteTenant(draft: InviteDraft): Promise<TenantWithContext> {
  ...
  // TODO(fáze 2): send invite email via Resend Edge Function
...
export async function resendInvite(tenantId: string): Promise<TenantWithContext> {
  ...
  // TODO(fáze 2): send new invite email via Resend
...
export async function createInvitation(
  ...
  // 3. Return token, expiry, and ready-to-send link
  return {
    token: inv.token,
    expires_at: inv.expires_at,
    invite_link: `https://app.estatiq.cz/auth/accept-invite?token=${inv.token}`,
  }
}
